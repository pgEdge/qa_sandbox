...
command = (f"cluster json-create {tmpcluster} {num_nodes} {dbname} {usr} {pw} --pg_ver={pgv} --port={port} --force")
...
print(f"returns.stdout contains: {returns.stdout}")
print(f"res = {res.returncode}\n")
print("*"*100)

if res.returncode == 0 and "validated" in returns.stdout:
