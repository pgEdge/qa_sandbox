...
for sub in subscriptions:
    n = n + 1
    ## Create a table:
    result = util_test.write_psql(f"CREATE TABLE case33 (bid integer PRIMARY KEY, bbalance integer, filler character(88))",host,dbname,port,pw,usr)
    ## Add a row:
    result = util_test.write_psql("INSERT INTO case33 VALUES (1, 11111, 'filler')",host,dbname,port,pw,usr)
    ## Add it to the default repset:
    result=util_test.run_cmd("comment", f"spock repset-add-table default case33 {dbname}", f"{cluster_dir}/n{n}")
...
row = util_test.write_psql("INSERT INTO case33 VALUES(11, 11000, null)",host,dbname,port1,pw,usr)
...
for sub in subscriptions:
    ...
    result = util_test.read_psql("SELECT * FROM case33;",host,dbname,port,pw,usr)
...
BEGIN
    ...
    INSERT INTO case33 VALUES (22, 22000, null);
END $$;
"""
...
row = util_test.write_psql("INSERT INTO case33 VALUES(22, 99000, null)",host,dbname,port1,pw,usr)
...
for sub in subscriptions:
    ...
    ## Confirm with spock sub-show-status
    status=util_test.run_cmd("Checking spock sub-show-status", (f"spock sub-show-status {sub} {dbname}"), f"{cluster_dir}/n{n}")
    print(f"Line 137 - The spock sub-show-status {sub} {dbname} command on n{n} returns: {status.stdout}")
    port = port + 1
...
for sub in subscriptions:
    ...
    ## Confirm with spock sub-show-status
    status=util_test.run_cmd("Checking spock sub-show-status", (f"spock sub-show-status {sub} {dbname}"), f"{cluster_dir}/n{n}")
    print(f"Line 170 - The spock sub-show-status {sub} {dbname} command on n{n} returns: {status.stdout}")
    port = port + 1
...
row = util_test.read_psql("SELECT remote_new_tup FROM spock.exception_log WHERE table_name = 'case33';",host,dbname,port2,pw,usr)
