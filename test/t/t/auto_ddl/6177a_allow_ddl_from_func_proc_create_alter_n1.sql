...
--creating the necessary pre-reqs and then switching to the appuser2 role
CREATE ROLE appuser2 LOGIN;
CREATE SCHEMA IF NOT EXISTS s617;

GRANT ALL PRIVILEGES ON SCHEMA public TO appuser2;
-- Grant execution rights to the non-superuser
GRANT EXECUTE ON FUNCTION public.get_table_repset_info(TEXT) TO appuser2;
GRANT ALL PRIVILEGES ON SCHEMA s617 TO appuser2;
DO $$
DECLARE
    db_name TEXT;
BEGIN
    ...
    -- Dynamically execute the GRANT command for appuser2
    EXECUTE format('GRANT CREATE ON DATABASE %I TO appuser2', db_name);
END $$;
