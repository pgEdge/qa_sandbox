...
import sys, os, util_test, subprocess, json, time
...
print(f"We're reading the .json file created on line 29: {cluster_dir}/{cluster_name}.json")
...
with open(f"{cluster_dir}/{cluster_name}.json", 'r') as file:
    data = json.load(file)
    #print(data)
    data["pgedge"]["pg_version"] = version
    data["node_groups"][0]["port"] = new_port_0
    data["node_groups"][1]["port"] = new_port_1
