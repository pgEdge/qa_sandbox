...
def run():
    ...
    for n in range(1,num_nodes+1):
        for z in range(1,num_nodes+1):
            for p in parallel_array:
                if n!=z:
                    ## Create Subs
                    cmd_node = f"spock sub-create sub_n{n}n{z}_{p} 'host=127.0.0.1 port={port_array[z-1]} user={repuser} dbname={db}' {db}"
    ...
    print(f"Line 49, print db: {db}")

    ## Sub Show Status Test
    cmd_node = (f"spock sub-show-status sub_n1n2_1 {db}")
