...
for sub in subscriptions:
    ...
    ## Create a table:
    result = util_test.write_psql(f"CREATE TABLE case11 (bid integer PRIMARY KEY, bbalance integer, filler character(88))",host,dbname,port,pw,usr)
    ## Add a row:
    result = util_test.write_psql("INSERT INTO case11 VALUES (1, 11111, 'filler')",host,dbname,port,pw,usr)
    ## Add it to the default repset:
    result=util_test.run_cmd("comment", f"spock repset-add-table default case11 {dbname}", f"{cluster_dir}/n{n}")
...
BEGIN
    ...
    INSERT INTO case11 VALUES (2, 70000, null);
END $$;
"""
...
for sub in subscriptions:
    n = n + 1
    ## Confirm with SELECT * FROM spock.tables.
    result = util_test.read_psql("SELECT * FROM case11;",host,dbname,port,pw,usr)
    print(f"Line 120 - SELECT * from spock.tables on node n{n} returns: {result}")
    port = port+1
    print("*"*100)    
...
row = util_test.write_psql("UPDATE case11 SET filler = 'hi' WHERE bid = 2",host,dbname,port1,pw,usr)
print(f"TEST STEP: The update to bid 2 returns: {row}")
print("*"*100)

## Demonstrate that replication continues on n1:
row = util_test.write_psql("UPDATE case11 SET filler = 'bye' WHERE bid = 1",host,dbname,port1,pw,usr)
...
for sub in subscriptions:
    ...
    result = util_test.read_psql("SELECT * FROM case11;",host,dbname,port,pw,usr)
    print(f"SELECT * from case11 on node n{n} returns: {result}")
    ## Confirm with spock sub-show-status
    status=util_test.run_cmd("Checking spock sub-show-status", (f"spock sub-show-status {sub} {dbname}"), f"{cluster_dir}/n{n}")
    print(f"Line 150 - The spock sub-show-status {sub} {dbname} command on n{n} returns: {status.stdout}")
    port = port + 1
...
row1 = util_test.read_psql("SELECT remote_new_tup FROM spock.exception_log WHERE table_name = 'case11';",host,dbname,port1,pw,usr)
...
result = util_test.read_psql("SELECT remote_new_tup FROM spock.exception_log WHERE table_name = 'case11';",host,dbname,port2,pw,usr)
print(f"Line 166 - TEST CONFIRMATION: SELECT * FROM spock.exception_log on n2 returns: {result}")
print("*"*100)


if '"value": 2, "attname": "bid", "atttype": "int4"' in str(result):
    util_test.EXIT_PASS()
else:
    util_test.EXIT_FAIL()
