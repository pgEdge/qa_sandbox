...
for sub in subscriptions:
    n = n + 1
    ## Create a table:
    result = util_test.write_psql(f"CREATE TABLE case2 (bid integer PRIMARY KEY, bbalance integer, filler character(88))",host,dbname,port,pw,usr)
    ## Add a row:
    result = util_test.write_psql("INSERT INTO case2 VALUES (1, 11111, 'filler')",host,dbname,port,pw,usr)
    ## Add it to the default repset:
    result=util_test.run_cmd("comment", f"spock repset-add-table default case2 {dbname}", f"{cluster_dir}/n{n}")
...
for sub in subscriptions:
    ...
    result = util_test.read_psql("SELECT * FROM case2;",host,dbname,port,pw,usr)
...
print(f"TEST STEP: bid 11 should be updated on n2, case2 contains: {row}")
print("*"*100)
